...
public class PaymentDetailService {
	...
	public PaymentDetailCriteriaResp find(PaymentDetailCriteriaReq req, boolean isReport, Integer sortDir, String sortField) throws Exception {
		try {
			...
			List<ColumnFormat> activeHeaders = getColumnFormatsActive(taskDetailHeaders, 1);
			List<ColumnFormat> activeDetHeaders = getColumnFormatsActive(taskDetailHeaders, 2);
			...
			columnFormatsPayment = getColumnFormatsActive(columnFormatsPayment, 1);
			...
			if(!StringUtils.isBlank(req.getContractNo())) {
				criteria.and(contractNoColumnPay).is(req.getContractNo());
			}
			...
			BasicDBObject fieldsFull = new BasicDBObject();
			BasicDBObject project = new BasicDBObject("$project", fieldsFull);
			fieldsFull.append(SYS_CREATED_DATE_TIME.getName(), 1);
			fieldsFull.append("sys_printedDateTime", 1);
			fieldsFull.append("sys_receiptNo", 1);
			fieldsFull.append(SYS_OWNER_ID.getName(), 1);
			fieldsFull.append("taskDetail._id", 1);
			fieldsFull.append("taskDetail." + SYS_OWNER.getName(), 1);
			...
			for (ColumnFormat columnFormat : columnFormatsPayment) {
				fieldsFull.append(columnFormat.getColumnName(), 1);
			...
			for (ColumnFormat columnFormat : activeHeaders) {
				if(columnFormat.getColumnName().equals(SysFieldConstant.SYS_OWNER.getName())) continue;

				if(!isReport) {
					fieldsFull.append("taskDetail." + columnFormat.getColumnName(), 1);
				}
			...
			if(isReport) {
				for (ColumnFormat columnFormat : activeDetHeaders) {
					fieldsFull.append("taskDetailFull." + columnFormat.getColumnName(), 1);
				}
			}
			...
			aggregateLst.add(Aggregation.match(criteria));

			BasicDBObject sorting;
			if(sortField == null) {
				sorting = new BasicDBObject("$sort", new BasicDBObject(StringUtils.isBlank(sortingColPayment) ? SYS_CREATED_DATE_TIME.getName() : sortingColPayment, -1));
			} else {
				sorting = new BasicDBObject("$sort", new BasicDBObject(sortField, sortDir));
			}
			aggregateLst.add(new CustomAggregationOperation(sorting));
			...
			aggregateLst.add(new CustomAggregationOperation(
		        new BasicDBObject(
			            ...
			            new BasicDBObject("from", NEW_TASK_DETAIL.getName())
			                .append("localField", contractNoColumnPay)
			                .append("foreignField", contractNoColumn)
			...
			//----------------------------------------------------------------------------------
			Aggregation agg = Aggregation.newAggregation(aggregateLst);
			AggregationResults<Map> aggregate = template.aggregate(agg, NEW_PAYMENT_DETAIL.getName(), Map.class);
			List<Map> paymentDetails = aggregate.getMappedResults();
			//----------------------------------------------------------------------------------

			resp.setHeaders(columnFormatsPayment);
			resp.setTaskDetailHeaders(activeHeaders);
	...
	//---------------------------------------: Private Zone :-----------------------------------------------
	private List<ColumnFormat> getColumnFormatsActive(List<ColumnFormat> columnFormats, int type) {
