angular.module('sbAdminApp').controller('PythonFileCtrl', function($rootScope, $scope, $state, $base64, $http, $localStorage, $translate, FileUploader, urlPrefix, loadData) {
	...
	$scope.deploy = function(id) {
		...
		$http.get(urlPrefix + '/restAct/program/deployPython?id=' + id).then(function(data) {	
	...
	uploader = $scope.uploader = new FileUploader({
        url: urlPrefix + '/restAct/program/uploadPython', 
 ...
 // FILTERS File type
    uploader.filters.push({
        name: 'customFilter',
        fn: function(item /*{File|FileLikeObject}*/, options) {
        	var isValid = item.name.endsWith(".py");        		        		
