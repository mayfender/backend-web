angular.module('sbAdminApp').controller('ForecastResultReportCtrl', function($rootScope, $scope, $state, $base64, $http, $localStorage, $translate, FileUploader, urlPrefix, loadData) {
	...
	$scope.search = function() {
		$http.post(urlPrefix + '/restAct/forecastResultReport/find', {
	...
	$scope.download = function(id) {
		$http.post(urlPrefix + '/restAct/forecastResultReport/download', {
			id: id,
			productId: $rootScope.workingOnProduct.id
	...
	$scope.updateEnabled = function(item) {
		$http.post(urlPrefix + '/restAct/forecastResultReport/updateEnabled', {
			id: item.id,
			productId: $rootScope.workingOnProduct.id
		}).then(function(data) {
			if(data.data.statusCode != 9999) {
	...
	$scope.updateTemplateName = function(item) {
		$http.post(urlPrefix + '/restAct/forecastResultReport/updateTemplateName', {
			id: item.id,
	...
	$scope.deleteItem = function(id) {
		...
		$http.post(urlPrefix + '/restAct/forecastResultReport/deleteFile', {
			id: id,
			currentPage: $scope.formData.currentPage, 
			itemsPerPage: $scope.formData.itemsPerPage,
			productId: $rootScope.workingOnProduct.id
	...
	uploader = $scope.uploader = new FileUploader({
        url: urlPrefix + '/restAct/forecastResultReport/upload', 
